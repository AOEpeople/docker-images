...
for CERTIFICATE in $(echo "${CERTIFICATES}"  | jq -r '.CertificateSummaryList[] | @base64'); do
    ...
    CERTIFICATION_DATA=$(aws --region "${AWS_DEFAULT_REGION}" acm describe-certificate --certificate-arn "${ARN}") || error_exit "Failed to get certification data"
    ...
    echo -e "aws_acm_ssl_certificate_expiration{domain=\"${DOMAIN}\", renewal_eligibility=\"${RENEWAL_ELIGIBILITY}\", arn=\"${ARN}\"} ${EXPIRES_IN_DAYS}" >> $TMPDIR/metrics.txt
done
