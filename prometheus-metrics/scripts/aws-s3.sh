...
if [ -z "${AWS_ACCESS_KEY_ID}" ] ; then error_exit "AWS_ACCESS_KEY_ID not set"; fi
if [ -z "${AWS_SECRET_ACCESS_KEY}" ] ; then error_exit "AWS_SECRET_ACCESS_KEY not set"; fi
...
s3list=`aws --region "${AWS_REGION}" s3 ls s3://"${AWS_BUCKET_NAME}"/ --recursive | sort | awk '{print $4}'` || error_exit "Failed list bucket"
for key in $s3list
do
    OBJECT_META_DATA=$(aws --region "${AWS_REGION}" s3api head-object --bucket "${AWS_BUCKET_NAME}" --key "${key}") || error_exit "Failed fetch object head"
