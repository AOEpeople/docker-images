...
if [ -z "${AWS_LAMBDA_JAR}" ] ; then error_exit "AWS_LAMBDA_JAR not set"; fi
...
AWS_LAMBDA_JAR=${AWS_LAMBDA_JAR//###VERSION_NUMBER###/$VERSION_NUMBER}
...
if [ "${DEBUG}" == "1" ]; then
    ...
    echo "AWS_LAMBDA_JAR: ${AWS_LAMBDA_JAR}"
...
aws s3 cp $AWS_LAMBDA_JAR s3://$AWS_BUCKET_NAME/$AWS_LAMBDA_JAR || error_exit "Failed to upload ${AWS_LAMBDA_JAR} to s3 bucket ${AWS_BUCKET_NAME}"
aws --region $AWS_REGION lambda update-function-code --function-name $AWS_LAMBDA_FUNCTION --s3-key $AWS_LAMBDA_JAR --s3-bucket $AWS_BUCKET_NAME || error_exit "Failed to update lambda function"