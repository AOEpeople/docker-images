...
function echoerr {
    set +x
    echo_red "============================================" 1>&2; echo_red "ERROR: $*" 1>&2; echo_red "============================================" 1>&2;
}
...
curl --silent $CURL_CREDENTIALS -O $ARTIFACT_URL || error_exit "Failed to download lambda artifact (${ARTIFACT_URL})"
...
aws \
    ...
    --s3-key "${AWS_LAMBDA_ARTIFACT_NAME}" \
    --s3-bucket "${AWS_BUCKET_NAME}" || error_exit "Failed to update lambda function"
